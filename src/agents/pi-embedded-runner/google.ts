...
>(params: {
  ...
  // for every model routed through Gemini-compatible tool schemas.
  if (!params.modelId?.toLowerCase().includes("gemini")) {
    return params.tools;
  }
...
export function logToolSchemasForGoogle(params: { tools: AgentTool[]; modelId?: string }) {
  if (!params.modelId?.toLowerCase().includes("gemini")) {
    return;
  }
