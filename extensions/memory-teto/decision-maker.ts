...
export class LLMDecisionMaker {
  ...
  async evaluate(journal: JournalData, delta: CleanMessage[]): Promise<DecisionResult> {
    ...
    this.logger.info(
      `memory-teto: LLMDecisionMaker.evaluate() - api=${this.api}, model=${this.model}, baseUrl=${this.baseUrl}`,
    );
    this.logger.info(
      `memory-teto: prompt length=${userPrompt.length}, context=${journal.cleanContext.length}, delta=${delta.length}`,
    );
    ...
    for (let iter = 0; iter < MAX_TOOL_ITERATIONS; iter++) {
      this.logger.info(`memory-teto: LLM iteration ${iter + 1}/${MAX_TOOL_ITERATIONS}...`);

      let response: AdapterResponse;
      try {
        response = await adapter.complete();
        this.logger.info(
          `memory-teto: LLM response - toolCalls=${response.toolCalls.length}, done=${response.done}`,
        );
      } catch (err) {
        this.logger.warn(`memory-teto: LLM API call failed: ${String(err)}`);
        if (err instanceof Error && err.stack) {
          this.logger.warn(`memory-teto: stack: ${err.stack}`);
        }
        throw err;
      }
      ...
      if (response.toolCalls.length === 0 || response.done) {
        ...
        this.logger.info(`memory-teto: LLM done after ${iter + 1} iterations, ${logs.length} ops`);
        break;
      }

      for (const tc of response.toolCalls) {
        this.logger.info(`memory-teto: executing tool ${tc.name} (id=${tc.id})`);
        const log = await this.handleToolCall(tc, journal.cleanContext.length, now, adapter);
        if (log) logs.push(log);
      }
    }
  ...
  private async handleToolCall(
    ...
    try {
      ...
      this.logger.warn(`memory-teto: tool ${tc.name} failed: ${resultText}`);
  ...
  private async executeTool(
    ...
    switch (name) {
      case "store_memory": {
        ...
        this.logger.info(`memory-teto: stored ${memoryId.slice(0, 8)}: "${text.slice(0, 60)}"`);
      ...
      case "forget_memory": {
        ...
        this.logger.info(`memory-teto: forgot ${memoryId.slice(0, 8)}`);
      ...
      case "update_memory": {
        ...
        this.logger.info(`memory-teto: updated ${memoryId.slice(0, 8)} → ${newId.slice(0, 8)}`);
