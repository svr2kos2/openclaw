...
function resolveDefaultDbPath(): string {
  const home = homedir();
  const preferred = join(home, ".openclaw", "memory", "teto");
  try {
  ...
  for (const legacy of LEGACY_STATE_DIRS) {
    const candidate = join(home, legacy, "memory", "teto");
    try {
...
export const memoryConfigSchema = {
  ...
  uiHints: {
    ...
    "chat.baseUrl": {
      ...
      help: "Base URL for the chat API (defaults based on selected backend)",
    },
    "chat.model": {
      label: "Chat Model",
      placeholder: DEFAULT_CHAT_MODEL,
      help: "Chat model for memory decisions (defaults based on selected backend)",
    },
    dbPath: {
      label: "Database Path",
      placeholder: "~/.openclaw/memory/teto",
      advanced: true,
    },
    autoCapture: {
      label: "Auto-Capture",
